...
<h2>ARTIKEL</h2><hr/>
<small><b>Tips & trick, Berita, dan lain - lain</b></small>
</div>
<?php 
	$batas = 5;
        if(isset($_GET['page']))
	...
	<div class="s-12 m-12 l-12 " style='margin:0px;float:none'>
		...
		<div style="background:#ff010"><h4 ><a href='<?=$url?><?=$up1->id_artikel?>.<?=$rep?>.htm' style='color:white;'><?=$up1->judul_artikel?></a> </h4></div>
			<p style='color:white'><?=$up1->waktu_artikel?> oleh <?=$up1->username?> Kategori : <b><?=$up1->jenis_artikel?></b></p>
			<p style='color:white'> 
...
<?php
   $q2=mysql_num_rows(mysql_query("select * from artikel where jenis_artikel!='loker'"));
            $jml=ceil($q2/$batas);
			for($i=1;$i<=$jml;$i++)
        {
            if($i==$page)
